...
class Solution {
    public Node correctBST(Node root) {
        ...
        int first=-1, mid=-1, second=-1;
        
        for(int ind=1; ind<in.size(); ind++){
            if(in.get(ind-1).data>in.get(ind).data){
                if(first==-1){
                    first = ind-1;
                    mid = ind;
                }
        ...
        if(second==-1) swap(first, mid, in);
        else swap(first, second, in);
        
        return root;
    }
    
    public void swap(int a, int b, List<Node> in){
        Node first = in.get(a), second = in.get(b);
        int temp;
    ...
    public void inorder(Node root, List<Node> in){
        if(root.left!=null) inorder(root.left, in);
        in.add(root);
